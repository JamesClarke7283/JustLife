...
func draw_creator() -> void:
	...
	if creator_tab=="Look":
		...
		for i in range(categories.size()):
			...
			var category_button=button(LifeCharacterIdentity.category_label(category),Vector2(1100+(i%3)*97,226+(i/3)*36),Vector2(89,32),func():set_creator_outfit_category(category),creator_outfit_category==category)
		...
		for i in range(outfit_names.size()):
			var b=button(outfit_names[i],Vector2(1100+(i%3)*97,392+(i/3)*36),Vector2(89,32),func():set_creator_clothing("outfit",i),int(profile.get("outfit",0))==i)
...
func swatches(colors:Array,key:String,p:Vector2,diameter:float,gap:float) -> void:
	for i in range(colors.size()):
		...
		var b=button("",p+Vector2(i*(diameter+gap),0),Vector2(diameter,diameter),func():
			profile[key]=c
...
func _place_memorial(id: String) -> void:
	if mode != "live" or not is_instance_valid(world):
		return
...
func _process(delta:float) -> void:
	...
	if mode=="live":
		...
		for member:Dictionary in household.members:
			...
			if int(player.profile.get("outfit",0))!=int(sim.character.get("outfit",0)) or int(player.profile.get("bottom",0))!=int(sim.character.get("bottom",0)) or str(player.profile.get("top_color",""))!=str(sim.character.get("top_color","")) or str(player.profile.get("bottom_color",""))!=str(sim.character.get("bottom_color","")) or str(player.profile.get("shoe_color",""))!=str(sim.character.get("shoe_color","")):
