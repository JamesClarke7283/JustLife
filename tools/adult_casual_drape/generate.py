"""Reproduce the reviewed casual drape and skin weights in a new isolated directory."""
import argparse
import json
import os
...
def main():
    ...
    source.add_argument('--repository', type=Path, help='Repository containing the pinned original-art commit')
    source.add_argument('--input-blend', type=Path, help='Byte-exact accepted fuller-lip Blend')
    ...
    if args.export and set(contract['expected_exports']) != set(VARIANTS):
    ...
    try:
        ...
        if sha(baseline) != contract['baseline']['sha256']:
            raise ValueError('Input differs from the pinned accepted fuller-lip Blend.')
        stages = contract['stages']
        drape = out / 'drape'
        run('drape', HERE / 'author_drape.py', ['--source', baseline, '--output', drape], 6)
        intermediate = drape / 'art/characters.blend'
        intermediate_pin = sha(intermediate)
        native_pins[intermediate] = intermediate_pin
        receipt['drape_container_byte_exact'] = intermediate_pin == stages['drape']['reviewed_native_container_sha256']
        require(intermediate, intermediate_pin)
        for name in ('object_facts', 'author_report'):
            require(drape / (name + '.json'), stages['drape'][name + '_sha256'])
        run('weights', HERE / 'author_weights.py', ['--source', intermediate,
            '--source-sha256', intermediate_pin, '--baseline-facts', drape / 'object_facts.json',
            '--output', out / 'candidate'], 1)
        require(intermediate, intermediate_pin)
        final = out / 'candidate/art/characters.blend'
        final_pin = sha(final)
        native_pins[final] = final_pin
        receipt['final_container_byte_exact'] = final_pin == stages['weights']['reviewed_native_container_sha256']
        require(final, final_pin)
        facts_path = out / 'candidate/object_facts.json'
        require(facts_path, stages['weights']['object_facts_sha256'])
        report_path = out / 'candidate/author_report.json'
        report = json.loads(report_path.read_text())
        if report['source_sha256'] != intermediate_pin:
            raise ValueError('Weight report must identify its actual intermediate input.')
        receipt['outputs']['candidate/author_report.json'] = sha(report_path)
        # Only container provenance can differ; retain the actual report unchanged.
        report['source_sha256'] = stages['drape']['reviewed_native_container_sha256']
        import hashlib
        comparison_pin = hashlib.sha256((json.dumps(report, indent=2) + '\n').encode()).hexdigest()
        receipt['weight_report_comparison'] = {'actual_source_sha256': intermediate_pin,
            'reviewed_source_sha256': report['source_sha256'],
            'only_adjusted_comparison_field': 'source_sha256', 'comparison_sha256': comparison_pin}
        write()
        if comparison_pin != stages['weights']['author_report_sha256']:
            raise ValueError('Weight report differs beyond its named input-container provenance.')
        # The pinned generated facts provide the complete native contract locally;
        # no redundant hundreds-of-object manifest is shipped with the source.
        facts = json.loads(facts_path.read_text())['after']
        if len(facts) != contract['native']['objects']:
            raise ValueError('Unexpected native object inventory.')
        native_contract = out / 'native_contract.json'
        object_pins = {name: hashlib.sha256(json.dumps(value, sort_keys=True,
            separators=(',', ':'), allow_nan=False).encode()).hexdigest() for name, value in facts.items()}
        native_contract.write_text(json.dumps({'native_contract': {'object_sha256': object_pins,
            'material_sha256': contract['native']['material_sha256']}}, indent=2) + '\n')
        run('native_reopen', HERE.parent / 'adult_eyes/verify_native.py', ['--source', final,
            '--contract', native_contract, '--report', out / 'native_reopen.json'], 1)
        require(final, final_pin)
        native = json.loads((out / 'native_reopen.json').read_text())
        if (native['source_sha256'] != final_pin or native['objects'] != len(object_pins)
        ...
        if args.export:
            for variant in VARIANTS:
                ...
                run(variant, HERE.parent / 'adult_eyes/export_variant.py', ['--source', final,
                    ...
                    '--variant', variant], 1)
                require(final, final_pin)
                require(out / 'candidate' / relative, contract['expected_exports'][variant])
        require(final, final_pin)
        require(intermediate, intermediate_pin)
        ...
        receipt['native_inputs_unchanged'] = all(path.is_file() and sha(path) == pin for path, pin in native_pins.items())
        if not receipt['input_unchanged'] or not receipt['tools_unchanged'] or not receipt['native_inputs_unchanged'] or receipt.get('original_input_unchanged') is False:
            receipt['status'] = 'failed'
        write()
    if receipt['status'] != 'complete':
        raise RuntimeError('Input or tools changed; result retained as failed.')
    print('ADULT_CASUAL_REPRODUCTION_COMPLETE', out)
    return 0
