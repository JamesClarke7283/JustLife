...
parser=argparse.ArgumentParser();parser.add_argument('--source',type=Path,required=True);parser.add_argument('--source-sha256',required=True);parser.add_argument('--output-root',type=Path,required=True);parser.add_argument('--variant',choices=['character_child_lod','character_child_broad_lod'],required=True)
 ...
 for o in descendants(root):
  ...
  if low:
   if o.type=='MESH' and o.data.shape_keys is None:
    ...
    if o.name=='Hair_Bob_Cap':
     assert [m.type for m in o.modifiers]==['SUBSURF','SOLIDIFY','DECIMATE']
...
print('ONE_CHILD_VARIANT_COMPLETE',args.variant)
