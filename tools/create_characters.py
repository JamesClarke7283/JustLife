...
# ---- Character sculpt / deformation pass 6 ---------------------------------
...
# Eyeballs sit almost flush with the flatter orbital plane. Their almond-shaped
# exposed surface has no raised outline or rim; skin naturally surrounds the edge.
eye_morph_objects=[]
def eye_blink(ob):
    ob=apply_mesh(ob);ob.shape_key_add(name='Basis');key=ob.shape_key_add(name='Blink')
    transform=ob.matrix_world.copy();inverse=transform.inverted()
    for i,v in enumerate(ob.data.vertices):
        point=transform@v.co;point.z=1.611+(point.z-1.611)*.035;point.y+=.014
        key.data[i].co=inverse@point
    eye_morph_objects.append(ob)
    return ob

def eye_disc(name,x,z,rad,y,ma):
    n=40;vs=[(x,y-.0007,z)]
    for j in range(n):
        a=2*pi*j/n;dz=max(-.0085,min(.0082,rad*sin(a)))
        vs.append((x+rad*cos(a),y,z+dz))
    return mesh(name,vs,[(0,j+1,(j+1)%n+1) for j in range(n)],ma,head,0)
for sign in [-1,1]:
    x=sign*.047;z=1.611;n=48;vs=[(x,-.108,z)]
    for j in range(n):
        t=2*pi*j/n;xx=.029*cos(t);zz=(.0105 if sin(t)>0 else .009)*sin(t)
        # The temporal corner follows the face around the orbit instead of floating.
        world_x=x+xx;front=.008-.108*max(.01,1-(world_x/.111)**2)**.15
        vs.append((world_x,front-.0013,z+zz+xx*.065*sign))
    white=mesh('Eyes_Sclera',vs,[(0,j+1,(j+1)%n+1) for j in range(n)],eye_white,head,1);eye_blink(white)
    eye_blink(eye_disc('Eyes_Iris_edge',x,z,.0098,-.1088,iris_edge))
    eye_blink(eye_disc('Eyes_Iris',x,z,.0084,-.1093,iris))
    eye_blink(eye_disc('Eyes_Pupil',x,z,.0045,-.1098,pupil))
    eye_blink(uv('Eyes_Catchlight',(x-.0028,-.1108,z+.0032),(.0015,.00035,.0015),catch,head,16,12))
...
bezline('Lips_Smile_seam',[(-.032,-.089,1.518),(-.018,-.099,1.514),(0,-.105,1.512),(.018,-.099,1.514),(.032,-.089,1.518)],.0006,nostril,head)
...
for s in [-1,1]:
    sleeve=loft('Outfit_Jacket_Sleeve',[(.875,.031,.032,-.008,s*.264),(.903,.037,.037,-.006,s*.263),(1.03,.047,.047,.01,s*.248),(1.17,.052,.054,.006,s*.229),(1.3,.067,.068,.005,s*.196),(1.382,.060,.059,.004,s*.154)],top,jacket,24,2);jacket_parts.append(sleeve)
...
for s in [-1,1]:
    loft('Outfit_Cardigan_Sleeve',[(.875,.032,.033,-.009,s*.264),(.91,.039,.039,-.007,s*.263),(1.07,.048,.049,.009,s*.245),(1.21,.055,.057,.006,s*.220),(1.34,.064,.064,.004,s*.176),(1.385,.045,.049,.004,s*.145)],top,cardigan,24,2)
...
def bind(ob,mode='garment'):
    ...
    for v in ob.data.vertices:
        ...
        if mode=='head':
            ...
            shin=1-smoothstep(.47,.63,z);hip=1-smoothstep(.875,1.025,z);right=smoothstep(-.035,.035,x)
    ...
    mod=ob.modifiers.new('Soft skeletal deformation','ARMATURE');mod.object=rig;mod.use_deform_preserve_volume=True
    return ob
# Retopology reduction preserves the sculpted silhouette without shipping voxel density.
for ob,target in [(headskin,18000),(pants,12000),(shirt,12000),(jacket_shell,16000)]+[(a,7000) for a in armskin]:
...
root['rig_version']=6;root['wardrobe_variants']='Outfit_Casual, Outfit_Jacket, Outfit_Cardigan'
...
def export(path):
    ...
    bpy.ops.export_scene.gltf(filepath=os.path.join(ROOT,path),export_format='GLB',use_selection=True,export_apply=True,export_yup=True,export_animations=False,export_morph=True,export_skins=True,export_cameras=False,export_lights=False)
export('assets/models/character_rig.glb')
...
root.scale.x=1.12
export('assets/models/character_broad_rig.glb')
root.scale.x=1.0
...
export('assets/models/character_rig_lod.glb')
root.scale.x=1.12;export('assets/models/character_broad_rig_lod.glb');root.scale.x=1.0
...
sc.view_settings.view_transform='AgX';sc.render.image_settings.file_format='PNG';sc.render.filepath=os.path.join(ROOT,'art/character_rig_preview.png');sc.render.film_transparent=False
...
cam.location=(.67,-3.7,1.93);cam.rotation_euler=(Vector((0,-.005,1.61))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.ortho_scale=.56;sc.render.resolution_x=900;sc.render.resolution_y=900;sc.render.filepath=os.path.join(ROOT,'art/character_rig_face.png');bpy.ops.render.render(write_still=True)
for style,group in [('bob',bob),('curls',curls)]:
    ...
    sc.render.filepath=os.path.join(ROOT,'art/character_rig_'+style+'_preview.png');bpy.ops.render.render(write_still=True)
for gr in (crop,bob,curls):
    for ob in descendants(gr):ob.hide_render=(gr!=crop)
cam.location=(2.5,-6.4,2.7);cam.rotation_euler=(Vector((0,0,.94))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.ortho_scale=2.06
...
for style,group in [('jacket',jacket),('cardigan',cardigan)]:
    for gr in (casual,jacket,cardigan):
        for ob in descendants(gr):ob.hide_render=(gr!=group)
    sc.render.filepath=os.path.join(ROOT,'art/character_rig_'+style+'.png');bpy.ops.render.render(write_still=True)
