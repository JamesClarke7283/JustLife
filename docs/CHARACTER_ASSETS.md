...
After promoting any asset or editable source, refresh both pinned records with `python3 tools/requalify_character_hashes.py` and confirm them with `--check`. That tool exists because editing them by hand already produced two real defects: four `.blend` manifest entries left at their pre-promotion hashes, and four verifier pins written as raw file digests where the verifier compares semantic digests. It has no bypass — it re-reads both records after writing and fails if anything still disagrees.

The promoted adult Bob replaces the previous Bob shell (nodes 31 → 5 in `character.glb`; every other mesh group is unchanged, total 403 → 377). `evidence/character63/bob_*.png` show a single continuous bob in place of the earlier separately-hanging clumps. This is a disclosed asset change and has not had its own independent accept/reject review.
