...
**Summit Dawn** is the game's own theme, in `assets/audio/Summit Dawn.wav` — 192.8 s, 48 kHz, stereo, ~−19 dBFS integrated RMS. It was supplied for the game and is used as-is; it is not generated by `tools/create_audio.py` and no third-party music is used.

`main.setup_music()` loads it with `AudioStreamWAV.load_from_file` (the same reason the voices do: playback never waits on Godot's asset importer), duplicates the stream, sets `loop_mode = LOOP_FORWARD` with `loop_begin = 0` and `loop_end = int(stream.get_length() * stream.mix_rate)`, and plays it at −8 dB on a plain `AudioStreamPlayer` named `SummitDawn`. The loop runs the whole file, so its own quiet opening (RMS ≈ 1 near 0.04 s) and its long quiet outro (the last ~0.5 s falls from RMS 21 to 10, ending within 1 % of silence) are the join; there is nothing loud to click on.
