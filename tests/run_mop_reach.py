"""Isolated original-shaft mop hand and planted-foot contact verification."""
...
for phase in ('import', 'mop'):
    ...
    if phase == 'import':
        ...
        if not args.capture:
            cmd += ['--headless']
        cmd += ['--script', 'res://tests/test_mop_reach.gd', '--', '--after']
        if args.capture:
            cmd += ['--capture']
    ...
    summaries = re.findall(r'Mop reach: (\d+) checks, (\d+) failures\.', output)
