...
def summarize(path):
    ...
    for member_id,stats in data['members'].items():
        ...
        last=max(((x['day']-1)*1440+x['minutes'] for x in events),default=480)
    ...
    fields=['id','name','stage','completed_actions','last_completion','hours_since_completion','waiting_hours','all_needs_zero_first_observed','final_action','final_phase','final_target']
