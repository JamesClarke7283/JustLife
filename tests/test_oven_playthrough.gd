...
func _start_bake()->void:
	...
	# Recipes draw from the household kitchen. A new home starts empty, so the
	# oven harness stocks it the same way the controller suite does: order then
	# collect, without waiting for the van's arrival clock.
	app.household.order_groceries("weekly")
	app.household.collect_groceries()
	if app.has_method("_sync_delivery_van"):app._sync_delivery_van()
	check(int(app.sim.skills.cooking.level)==4,"Explicit fixture XP unlocks Cooking level4 before any cooking.")
	check(LifeGroceries.can_cook(app.household.groceries),"Fixture kitchen stock lets Cooking open the cookbook.")
	...
	expected={"funds_before":app.household.funds,"stock_before":LifeGroceries.stock(app.household.groceries),"wants_before":app.sim.wants.duplicate(true)}
	...
	observe_oven=true;observing_motion=true
	# Very fast covers the walk; drop to normal before the short load-transfer
	# window so a large rendered frame cannot skip the mid-tray pose.
	await press("▶▶▶")
	if not await wait_until(func()->bool:return active_is("cook") and float(app.sim.get_current_action().get("progress",1.0))>=.18,"real physical approach to the oven",45):return
	...
	check(bool(current.paid) and app.household.funds==int(expected.funds_before) and LifeGroceries.stock(app.household.groceries)==int(expected.stock_before)-1,"Actual arrival draws one meal from the kitchen without a purse charge.")
	await press("▶")
	if not await wait_until(_mid_load,"actual dish insertion midway between held and rack transforms",120):return
...
func _mid_load()->bool:
	return active_is("cook") and _phase()=="load" and _transfer_fraction()>=.2 and _transfer_fraction()<=.85
...
func _assert_preparation(label:String)->void:
	...
	check(app.household.funds==int(expected.funds_before) and app.household.meals.batches.is_empty(),label+": kitchen stock paid the meal and no prematurely edible batch exists.")
...
func _save_checkpoint(title:String,index:int)->void:
	...
	var state:Dictionary=_snapshot();state.funds_before=expected.funds_before;state.stock_before=expected.get("stock_before",LifeGroceries.stock(app.household.groceries)+1);state.wants_before=expected.wants_before.duplicate(true);state.save_id=app.active_save_id;state.stage=index
...
func _finish_bake()->void:
	await press("▶▶▶")
	if not await wait_until(func()->bool:return active_is("cook") and float(app.sim.get_current_action().get("progress",0.0))>=.78,"closed bake approaching extraction",120):return
	await press("▶")
	if not await wait_until(func()->bool:return active_is("cook") and _phase()=="unload" and _transfer_fraction()>.2 and _transfer_fraction()<.8,"actual extraction from the oven",120):return
	...
	await press("▶▶▶")
	if not await wait_until(func()->bool:return "player:read" in member_completed,"the queued later instruction completes after serving and any chosen portion",180):return
	await press("Ⅱ");await frames(3)
...
func _assert_completed_wallet(label:String)->void:
	...
	check(wallet_events.filter(func(event:Dictionary)->bool:return int(event.delta)<0).is_empty(),label+": cooking draws kitchen stock, so the purse sees no ingredient debit.")
	...
	check(app.household.funds==int(expected.funds_before)+60,label+": the wallet accounts for the independently verified earned reward alone.")
...
func _write_oven_report()->void:
	var result:Dictionary={"stage":oven_stage,"assertions":assertions,"failures":failures,"events":oven_events,"evidence":evidence,"trace":oven_trace,"max_motion_excess":motion_excess,"wallet_events":wallet_events,"method":"Actual rendered frames, public UI signals, three fresh processes. Only initial fixture writes: autonomy=false, Cooking XP450, and weekly groceries ordered then collected; no need/clock/progress/arrival injection."}
