...
func run()->void:
 ...
 actor.configure({"name":"Milo", "age_stage":"child", "low_detail":true,"hair":1,"outfit":0,"skin_color":"bf8867","top_color":"7295a3"})
 actor.voice_enabled=false
 block(Vector3(0,.57,0),Vector3(.47,.18,.45),Color("b38a6c"))
 block(Vector3(0,.82,-.27),Vector3(.47,.46,.10),Color("b38a6c"))
 block(Vector3(0,.91,.64),Vector3(1.1,.09,.65),Color("c39e79"))
 block(Vector3(0,.968,.45),Vector3(.36,.025,.12),Color("3d514e"))
 block(Vector3(0,1.17,.74),Vector3(.46,.30,.035),Color("586f77"))
 actor.set_activity_anchor(Vector3(0,.66,0),0,"seat","school",{"hand_center":Vector3(0,.99,.34)})
 for i:int in range(180):actor.animate(1.0/60.0,1,false,"school");await process_frame
 camera.size=2.1;camera.position=Vector3(2.2,1.8,2.5);camera.look_at(Vector3(0,.87,.12));label.text="JustLife • Child • Online classes"
 await process_frame;await RenderingServer.frame_post_draw
 root.get_texture().get_image().save_png("res://art/child_school.png")
 actor.clear_activity_anchor();actor.position=Vector3(0,0,0)
 await capture("snack",1.7,true)
 print("Age gallery saved: ",label_prefix)
