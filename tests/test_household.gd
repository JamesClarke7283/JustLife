...
func run() -> void:
	...
	check(home.funds==2606,"Bills are charged once for the entire household")
	...
	check(restored.members.size()==2 and restored.funds==2606,"Save retains all Lifelets and shared money")
	...
	check(restored.members.size()==2 and restored.funds==2606,"Failed restore leaves household untouched")
