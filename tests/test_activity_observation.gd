...
func _phase_observation()->void:
	# Very-fast natural spends the first107 calls waiting on the easel, then —
	# only after the30-minute queue policy — replans onto Read and arrives.
	# Row106 is therefore an active paid Read with the approach route retired;
	# the old route95/.05 observation capture was a normal-speed artifact that
	# danger-interrupt policy (e6fcfc3) and the very-fast natural budget retired.
	check(audit.steps.size()==107 and int(audit.steps[-1].step)==106,"Same public very-fast prefix stops after original row106,107 advancing calls.")
	var sim:LifeSim=app.household.member_sim("housemate_1")
	var action:Dictionary=sim.get_current_action()
	check(str(action.id)=="read" and str(action.phase)=="active" and bool(action.paid) and float(action.elapsed)>0.0,"Actual Ellis has arrived at Read after the occupied-easel queue wait.")
	check(not app.traversal.routes.has("housemate_1"),"The finished Read approach retires its traversal route before the named save.")
	...
	check(not app.traversal.routes.has("housemate_1"),"Paused materialization leaves Ellis without a fabricated post-arrival route.")
	await _public_save("Finite queue-replan — actual Ellis Read");_exclude_inputs()
	...
	check(not app.traversal.routes.has("housemate_1"),"Actual named save retains the retired post-arrival journey exactly.")
	if bool(disk.ok):
...
func _fresh_observation()->void:
	...
	for saved:Dictionary in read.data.members:
		var member:LifeSim=app.household.member_sim(str(saved.id))
		check(member.needs==saved.state.needs and _same(_decoded_career(saved.state.career),member.career),"Fresh exact decoded needs including Fun, and career: "+str(saved.id))
	...
	check(str(action.id)=="read" and str(action.phase)=="active" and bool(action.paid) and float(action.elapsed)>0.0,"Fresh actual Read resumes paid active progress, not an invented unpaid approach.")
	check(not app.traversal.routes.has("housemate_1"),"Fresh reconstruction keeps the retired post-arrival journey without inventing a route.")
	...
	var origin:Vector3=app.world.actors.housemate_1.position
	...
	audit.origin=origin;audit.original_read=action_before;audit.original_action_retained_at_end=is_same(sim.get_current_action(),action);audit.minimum_clearance=minimum;audit.final=_busy_record();audit.final_state=_query_state()
