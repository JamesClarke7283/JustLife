...
func _clearing_and_pending_host()->void:
	...
	check(float(f.action.elapsed)==float(f.elapsed) and app.household.funds==oven_baseline,"Detached pose reconstruction preserves cooking progress and draws no further meal.")
