...
func _run()->void:
	...
	check(app.sim.funds==3750,"Fresh siblings enter the cottage with the scaled two-person purse and no unearned family friendship bonus.")
	...
	await press("Ⅱ")
	check(app.sim.funds==3750 and not _fresh_friend(app.sim).complete,"Partial conversation grants no money or premature wish completion.")
	await press("Cancel action");await press("▶");await frames(12);await press("Ⅱ")
	check(app.sim.funds==3750 and not _fresh_friend(app.sim).complete,"Canceling a partial conversation preserves the unfinished reward.")
	await _open_social("housemate_1");await press(str(app.sim.get_action_definition("friendly").label));await press("▶")
	...
	check(app.sim.funds==3910,"One completed mutual conversation pays exactly80 for each participant, once.")
	...
	await press("Ⅱ")
	check(app.sim.funds==3910,"Repeating the conversation does not pay the same fulfilled goal again.")
